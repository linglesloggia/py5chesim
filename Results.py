...
class UEgroup:
    ...
    def setReq(self, delay, avl):
        ...
        self.req["reqThroughputDL"] = (
            (8000 * self.p_sizeDL) / (1024 * 1024 * self.p_arr_rateDL)
        )
        self.req["reqThroughputUL"] = 0  # 8*self.p_sizeUL*self.p_arr_rateUL
    ...
    def activateSliceScheds(self, interSliceSche, env):
        ...
        if self.num_usersDL > 0:
            procSchDL = env.process(
                interSliceSche.slices[self.label].schedulerDL.queuesOut(env)
            )
        if self.num_usersUL > 0:
            procSchUL = env.process(
                interSliceSche.slices[self.label].schedulerUL.queuesOut(env)
            )
    ...
    def printSliceResults(self, interSliceSche, t_sim, bw, measInterv):
        ...
        if self.num_usersDL > 0:
            ...
            printResults(
                ...
                self.usersDL,
                self.num_usersDL,
                interSliceSche.slices[self.label].schedulerDL,
                t_sim,
                True,
                False,
                self.sinr_0DL,
            ...
            [SINR_DL, times_DL, mcs_DL, rU_DL, plr_DL, th_DL] = getKPIs(
                "DL",
                "Statistics/dlStsts" + "_" + self.label + ".txt",
                self.usersDL,
                self.num_usersDL,
                self.sinr_0DL,
                measInterv,
            ...
            [times_DL, rU_DL, plr_DL, th_DL, cnx_DL, buf_DL, met] = getKPIsInter(
                "DL",
                "Statistics/dlStsts_InterSlice.txt",
                list(interSliceSche.slices.keys()),
        ...
        if self.num_usersUL > 0:
            printResults(
                ...
                self.usersUL,
                self.num_usersUL,
                interSliceSche.slices[self.label].schedulerUL,
                t_sim,
                True,
                False,
                self.sinr_0UL,
            ...
            # print('Configured Signalling Load: '+str(interSliceSche.slices[self.label].signLoad))
            # print('Using Robust MCS: '+str(interSliceSche.slices[self.label].robustMCS))
            [SINR_UL, times_UL, mcs_UL, rU_UL, plr_UL, th_UL] = getKPIs(
                "UL",
                "Statistics/ulStsts" + "_" + self.label + ".txt",
                self.usersUL,
                self.num_usersUL,
                self.sinr_0UL,
                measInterv,
            ...
            [times_UL, rU_UL, plr_UL, th_UL, cnx_UL, buf_UL, met] = getKPIsInter(
                "UL",
                "Statistics/ulStsts_InterSlice.txt",
                list(interSliceSche.slices.keys()),
                len(list(interSliceSche.slices.keys())),
            )
            makePlotsInter(
...
def getKPIs(dir, stFile, users, num_users, sinr_0, measInterv, tSim):
    ...
    for line in lines[1:]:
        ...
        if (sent[ue][len(sent[ue]) - 1] - sent[ue][len(sent[ue]) - 2]) > 0:
            plr[ue].append(
                100
                * float(lost[ue][len(lost[ue]) - 1] - lost[ue][len(lost[ue]) - 2])
                / (sent[ue][len(sent[ue]) - 1] - sent[ue][len(sent[ue]) - 2])
            )
        else:
            plr[ue].append(0)
    ...
    #write_to_file(users, SINR, times, mcs, rU, plr, th)
    return SINR, times, mcs, rU, plr, th

def write_to_file(users, SINR, times, mcs, rU, plr, th):
...
def getKPIsInter(dir, stFile, slices, num_slices):
    ...
    for line in lines[1:]:
        ...
        rU[slice].append(int(float(resUse)))
        buf[slice].append(int(buff))
        met[slice].append(float(float((mt))))
        sent[slice].append(int(float(sntPackets)))
        lost[slice].append(int(float((lstPackets))))
        rBytes[slice].append(int(float((rcvdBytes))))
        if (sent[slice][len(sent[slice]) - 1] - sent[slice][len(sent[slice]) - 2]) > 0:
            plr[slice].append(
                100
                * float(
                    lost[slice][len(lost[slice]) - 1]
                    - lost[slice][len(lost[slice]) - 2]
                )
                / (
                    sent[slice][len(sent[slice]) - 1]
                    - sent[slice][len(sent[slice]) - 2]
                )
            )
        else:
            plr[slice].append(0)
        rcvdBytes = (
            rBytes[slice][len(rBytes[slice]) - 1]
            - rBytes[slice][len(rBytes[slice]) - 2]
        )
        deltaT = (
...
def save_(th, time, u, file_path, slice='None'):
    # Create a pandas dataframe with the input data
    df = pd.DataFrame({
        "x": time,
        "y": th,
    ...
    if not os.path.exists('Statistics/process'):
    # Create the directory
        os.makedirs('Statistics/process')


    # Write the dataframe to a file in columns, if the filename exists, creates new file with a number
    df.to_csv(file_path, index=False, sep=' ', mode='a')


def makeTimePlot(u, plotName, plotType, xAx, yAx, slcLbl = 'non'):
    lettersize = 12
    ...
    if plotType == "UEresUse":
        plt.plot(xAx[u], yAx[u], label="Recursos Usados(%) por " + u)
        save_(yAx[u], xAx[u], u, 'Statistics/process/UEresUse')
    if plotType == "sliceResUse":
        plt.plot(xAx[u], yAx[u], label="Recursos asignados a " + u)
        save_(yAx[u], xAx[u], u, 'Statistics/process/sliceResUse')
    if plotType == "SINR":
        plt.plot(xAx[u], yAx[u], label="SINR(dB) " + u)
    if plotType == "MCS":
        plt.scatter(xAx[u], yAx[u], label="MCS " + u)
    if plotType == "Connections":
        plt.plot(xAx[u], yAx[u], label="Conexiones " + u)
    if plotType == "PBuff":
        # plot xAx[u] and yAx[u] with a label with big letters
...
def makeIntraSlicePlot(plotName, plotType, xAx, yAx, slcLbl, bw, sch, gran):
    ...
    for ue in list(xAx.keys()):
        makeTimePlot(ue, plotName, plotType, xAx, yAx, slcLbl)
    ax = plt.gca()
    plt.autoscale()
    ax.xaxis.set_major_locator(MultipleLocator(CADA))#gran * 3))
    ax.grid(which="major", color="#CCCCCC", linestyle="--")
    lettersize = 24

    plt.legend( prop={"size": lettersize})
    ...
    ''' plt.title(
        plotName
        + " | BAND="
        + str(bw)[1 : len(str(bw)) - 1]
        + " MHz | "
        + str(len(list(xAx.keys())))
        + " UEs | "
        + slcLbl
        + " slice | "
        + str(sch)
...
def makeInterSlicePlot(plotName, plotType, xAx, yAx, bw, sch, gran):
    ...
    plt.savefig(
        "Figures/" + fileName + "|BW=" + str(bw) + "|InterSliceSched=" + str(sch)
    )

    # also save the fig in pdf format
    plt.savefig(
        "Figures/" + fileName + "|BW=" + str(bw) + "|InterSliceSched=" + str(sch) + ".svg"
    )
...
def makePlotsIntra(dir, times, sinr, mcs, rU, plr, th, slcLbl, bw, sch, gr):
    ...
    makeIntraSlicePlot("TH"  + " (Mbps)", "TH", times, th, slcLbl, bw, sch, gr)
    makeIntraSlicePlot(
        "PRBs"  + " (%)", "UEresUse", times, rU, slcLbl, bw, sch, gr
    )
...
def makePlotsInter(dir, times, res, plr, th, cnx, buf, met, bw, schIn, gr):
    ...
    #print(times)
    makeInterSlicePlot("TH"  + " (Mbps)", "TH", times, th, bw, schIn, gr)
    makeInterSlicePlot("PRBs" , "sliceResUse", times, res, bw, schIn, gr)
    makeInterSlicePlot("Connections-" , "Connections", times, cnx, bw, schIn, gr)
    makeInterSlicePlot("Paquetes en Buffer" , "PBuff", times, buf, bw, schIn, gr)
