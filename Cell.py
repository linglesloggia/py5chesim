...
class Cell:
    ...
    def __init__(self, i, b, fr, dm, mBue, tdd, gr, schInter):
        ...
        elif schInter[0:2] == "RL":
            self.interSliceSched = rlp_Scheduler(self.bw, fr, dm, tdd, gr)
        elif schInter[0:2] == "AX":
    ...
    def updateStsts(self, env, interv, tSim):  # ---------- PEM -------------
        ...
        for slice in list(self.interSliceSched.slices.keys()):
            ...
            self.slicesStsts[slice]["DL"] = open(
                "Statistics/dlStsts" + "_" + slice + ".txt", "w"
            )
            self.slicesStsts[slice]["DL"].write(
                "time ue sinr MCS BLER ResourceUse sntPackets lstPackets rcvdBytes sliceLabel"
                + "\n"
            )
            self.slicesStsts[slice]["UL"] = open(
                "Statistics/ulStsts" + "_" + slice + ".txt", "w"
            )
            self.slicesStsts[slice]["UL"].write(
                "time ue sinr MCS BLER ResourceUse sntPackets lstPackets rcvdBytes sliceLabel"
                + "\n"
            )
        self.slicesStsts["InterSlice"] = {}
        self.slicesStsts["InterSlice"]["DL"] = open(
            "Statistics/dlStsts_InterSlice.txt", "w"
        )
        self.slicesStsts["InterSlice"]["DL"].write(
            "time Slice Connections ResourceUse sntPackets lstPackets rcvdBytes bufferSize"
            + "\n"
        )
        self.slicesStsts["InterSlice"]["UL"] = open(
            "Statistics/ulStsts_InterSlice.txt", "w"
        )
        self.slicesStsts["InterSlice"]["UL"].write(
            "time Slice Connections ResourceUse sntPackets lstPackets rcvdBytes bufferSize"
            + "\n"
        )
        ...
        while env.now < (tSim * 0.83):
            ...
            for slice in list(self.interSliceSched.slices.keys()):
                conn_UEs = list(
                    self.interSliceSched.slices[slice].schedulerDL.ues.keys()
                )
                ...
                for ue in conn_UEs:
                    lP = (
                        self.interSliceSched.slices[slice]
                        .schedulerDL.ues[ue]
                        .packetFlows[0]
                        .lostPackets
                    )
                    ...
                    resUse = (
                        self.interSliceSched.slices[slice].schedulerDL.ues[ue].resUse
                    )
                ...
                if slice != "LTE":
                    conn_UEs = list(
                        self.interSliceSched.slices[slice].schedulerUL.ues.keys()
                    )
                    ...
                    for ue in list(
                        ...
                        bler = (
                            self.interSliceSched.slices[slice].schedulerUL.ues[ue].bler
                        )
                        resUse = (
                            self.interSliceSched.slices[slice]
                            .schedulerUL.ues[ue]
                        ...
                        buf = self.interSliceSched.slices[
                            slice
                        ].schedulerUL.updSumPcks()
                        lostP = lostP + lP
                        sentP = sentP + sP
                        recByts = recByts + rB
                        self.slicesStsts[slice]["UL"].write(
                    ...
                    if (
                        self.sch[0:2] == "PF"
                        and len(self.interSliceSched.slices[slice].rcvdBytes)
                        >= self.interSliceSched.rcvdBytesLen
                    ):
                        self.interSliceSched.slices[slice].rcvdBytes.popleft()
                    self.interSliceSched.slices[slice].rcvdBytes.append(
                        sliceRcvdBytes
                    )  # to consider the received bytes during the meassurement interval
