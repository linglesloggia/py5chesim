...
class InterSliceScheduler:
    ...
    def __init__(self, ba, fr, dm, tdd, gr):
        ...
        self.nRBtable = {
            ...
            "FR2": {
                "60kHz": {"50MHz": 66, "100MHz": 132, "200MHz": 264},
                "120kHz": {"50MHz": 32, "100MHz": 66, "200MHz": 132, "400MHz": 264},
            },
    ...
    def resAlloc(self, env):  # PEM ------------------------------------------------
        ...
        while True:
            # print(self.slices[list(self.slices.keys())[0]])
    ...
    def createSlice(
        self, dly, thDL, thUL, avl, cnxDL, cnxUL, ba, dm, mmd, ly, lbl, sch
    ):
        """This method creates a slice and stores it in the slices dictionary."""
        self.slices[lbl] = Slice(
            dly, thDL, thUL, avl, cnxDL, cnxUL, ba, dm, mmd, ly, lbl, self.tdd, sch
        )
    ...
    def printSliceConfig(self, slice):
        ...
        if slice == "LTE":
            ...
            sliceBrBuffSz = max(
                ...
                self.slices[slice].schedulerUL.updSumPcks(),
            )
            sliceRessources = max(
                self.slices[slice].schedulerDL.nrbUEmax,
                self.slices[slice].schedulerUL.nrbUEmax,
            )
            self.dbFile.write("<h5> Slice " + self.slices[slice].label + " data:</h5>")
            self.dbFile.write(
                "DL/UL symbols: "
                + str(self.slices[slice].schedulerDL.TDDsmb)
                + "/"
                + str(self.slices[slice].schedulerUL.TDDsmb)
                + "<br>"
            )

        self.dbFile.write("Connections: " + str(sliceCnx) + "<br>")
        self.dbFile.write(
            "Bearer Buffer Size (Packets): " + str(sliceBrBuffSz) + "<br>"
        )
