...
from lib.SchedulerUtils import loadModTable, loadSINR_MCStable, loadtbsTable, getTbs
...
class IntraSliceScheduler:
    ...
    def __init__(
        ...
        self.sinrModTable = loadSINR_MCStable(self.tdd)  # 5G
        self.ind_u = 0
        self.nrbUEmax = n
        self.sbFrNum = 0
        self.dbMd = debMd
        ...
        while True:
            ...
            if len(self.queue.res) > 0:
                for i in range(len(self.queue.res)):  # [0])):
                    ...
                    if random.random() <= (1.0 - self.ues[ue].bler) or (
                        tbl.reTxNum > 0
                    ):  # not sending again retransmitted TB
                        self.printDebDataDM(
                            '<p style="color:green">'
                            + ue
                            + " TB "
                ...
                self.printDebDataDM(
                    '<p style="color:green">' + "no more TBs in queue" + "</p>"
                )
    ...
    def updSliceQoeStats(self, rcvdBytes):
        if rcvdBytes * 8 * 1000 / (1 * 1024 * 1024) > self.rateSLA:
            self.sliceQoE = 1
        else:
            self.sliceQoE = 0
    ...
    def queueUpdate(self):
        ...
        self.rb_assigned = self.resAlloc(self.nrbUEmax)
        rb = 0
        if self.mimomd == "MU":
        ...
        while (
            ...
            and packts > 0
            and self.rb_lim > 0
            and (rb + self.ues[self.ueLst[self.ind_u]].prbs) <= self.rb_lim
        ):
        ...
        # users_satisfied = 0
        """
        for uue in self.ueLst:
            if self.ues[uue].throughput >= self.throughput_sla:
                self.ues[uue].satisfied = True
                users_satisfied += 1
        """
        # print('usuarios: ',len(self.ueLst), 'NSRS ', self.NSRS, 'PRBs ', self.nrbUEmax)
        # self.NSRS = users_satisfied / len(self.ueLst) if len(self.ueLst) != 0 else 0
    ...
    def updSumPcks(self):  # Update packts variable (sum all packets in bearer buffers)
        ...
        for uue in list(self.ues.keys()):
            packets = packets + len(
                self.ues[uue].bearers[0].buffer.pckts
            )  # + len(self.ues[uue].bearers[1].buffer.pckts)
        # print(packets)
        return packets
    ...
    def moving_average(self, current_utilization, u):
        if len(self.ues[u].utilization_list) > 10:
            self.ues[u].utilization_list.pop(0)
...
#------------------------------>
    ...
    def printQue(self):
        ...
        if len(list(self.ues.keys())) > 0:
            for ue in list(self.ues.keys()):
                self.printDebData(
                    '<p style="color:blue">' + ue + " DRB queue:" + "</p>"
                )
                if (
                    len(self.ues[ue].bearers) > 0
    ...
    def printResAlloc(self):
        if self.dbMd:
            ...
            for ue in list(self.ues.keys()):
                resAllocMsg = (
                    resAllocMsg + ue + ": " + str(self.ues[ue].prbs) + " PRBs" + "<br>"
                )
...
class LTE_scheduler(IntraSliceScheduler):
    ...
    def __init__(self, ba, n, debMd, sLod, mmd_, ly_, dir):
        IntraSliceScheduler.__init__(
            self, ba, n, debMd, sLod, 1, mmd_, ly_, dir, 14, False, "LTE", "RR"
        )
        self.BER = 0.01
        self.modTable = []
        self.tbsTable = []
    ...
    def loadModTable(self):
        ...
        self.modTable.append(
            {
                "spctEff": 0.3056640625,
                "bitsPerSymb": 2,
                "codeRate": 0.1528320313,
                "tbsi": 4,
                "mod": "4-QAM",
            }
        )
        ...
        self.modTable.append(
            {
                "spctEff": 0.4892578125,
                "bitsPerSymb": 2,
                "codeRate": 0.2446289063,
                "tbsi": 6,
                "mod": "4-QAM",
            }
        )
        ...
        self.modTable.append(
            {
                "spctEff": 0.7392578125,
                "bitsPerSymb": 2,
                "codeRate": 0.3696289063,
                "tbsi": 8,
                "mod": "4-QAM",
            }
        )
    ...
    def loadBlerTable(self):
        ...
        self.blerTable["BER=0.016"].append(
            {
                "24": 0.475235796886,
                "22": 0,
                "20": 0,
        ...
        self.blerTable["BER=0.016"].append(
            {
                "24": 0.000563804697832,
                "22": 0.000563804697832,
                "20": 0.000563804697832,
                "18": 0.000563804697832,
                "16": 0.000563804697832,
                "14": 0,
                "12": 0,
                "10": 0,
                "8": 0,
        ...
        self.blerTable["BER=0.016"].append(
            {
                "24": 0.000238631370249,
                "22": 0.000238631370249,
                "20": 0.000238631370249,
                "18": 0.000238631370249,
                "16": 0.000238631370249,
                "14": 0.000238631370249,
                "12": 0,
                "10": 0,
                "8": 0,
                "6": 0,
        ...
        self.blerTable["BER=0.016"].append(
            {
                "24": 0.000603065635079,
                "22": 0.000603065635079,
                "20": 0.000603065635079,
                "18": 0.000603065635079,
                "16": 0.000603065635079,
                "14": 0.000603065635079,
                "12": 0.000603065635079,
                "10": 0.000603065635079,
                "8": 0.000603065635079,
                "6": 0,
                "4": 0,
                "2": 0,
        ...
        self.blerTable["BER=0.01"].append(
            {
                "24": 0,
                "22": 0,
                "20": 0,
        ...
        self.blerTable["BER=0.01"].append(
            {
                "24": 0.000147710487445,
                "22": 0.000147710487445,
                "20": 0.000147710487445,
                "18": 0.000147710487445,
                "16": 0,
                "14": 0,
                "12": 0,
                "10": 0,
        ...
        self.blerTable["BER=0.01"].append(
            {
                "24": 0.000399581971195,
                "22": 0.000399581971195,
                "20": 0.000399581971195,
                "18": 0.000399581971195,
                "16": 0,
                "14": 0,
                "12": 0,
                "10": 0,
        ...
        self.blerTable["BER=0.01"].append(
            {
                "24": 0.000108108108108,
                "22": 0.000108108108108,
                "20": 0.000108108108108,
                "18": 0.000108108108108,
                "16": 0.000108108108108,
                "14": 0,
                "12": 0,
                "10": 0,
                "8": 0,
    ...
    def setBLER(self, u):  # BLER calculation
        if (self.BER < 0.01) or (self.BER == 0.01 and self.PRBs > 6):
            ...
            if mcs <= 24:
                pend = (
                    self.blerTable[berKey][sinrs_i][str(mcs)]
                    - self.blerTable[berKey][sinrs_i - 1][str(mcs)]
                ) / (sinrs[sinrs_i] - sinrs[sinrs_i - 1])
                self.ues[u].bler = self.blerTable[berKey][sinrs_i][str(mcs)] + pend * (
                    sinr - sinrs[sinrs_i]
                )
            else:
